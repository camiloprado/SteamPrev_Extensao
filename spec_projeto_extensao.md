...
2. **Camada de Apresentação (Frontend/Bot)**: 
   - A interface do usuário. Pode ser um Bot do Discord ou um Dashboard interativo (Streamlit) ou uma extensao de navegador.
...
* **Interface (Opções)**:
  ...
  * *Opção C (Extensão)*: `manifest.json`, `background.js`, `popup.html`, `popup.js`. (Para o navegador).
...
* `POST /predict/game`
  ...
  * **Resposta**: `game`, `classificacao`, `regressao` (dias, desconto previsto, MAE, preço estimado). `features_utilizadas` só com `?debug=true`.
* `POST /predict/classificacao` e `POST /predict/regressao`
  * Modo avançado: recebe as 18 features já calculadas (`GameFeaturesInput`).
* `GET /docs` / `GET /redoc`
